...
exports.changeBlockStatus = async(req,res)=> {
  try{
    ...
    if(!id){
      return res.status(404).json({ success: false, message: "Emplpoyee id required" });
    }
    ...
    const isBlockStatus = !user.isBlock;
    user.isBlock=isBlockStatus;
    await user.save();

    res.json({
      success: true,
      message: `${user.role} ${
          isBlockStatus ? "blocked" : "unBlocked"
        } successfully.`,
    });
  } catch(error) {
